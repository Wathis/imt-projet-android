...
class BooksActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        floatingButton.setOnClickListener {
            ...
            intent.putParcelableArrayListExtra(BasketActivity.bookExtra, ArrayList(this.books.filter { book -> (book?.quantity?: 0) > 0 }))
        ...
        booooks?.enqueue(object : Callback<List<Book?>?> {
            ...
            override fun onResponse(call: Call<List<Book?>?>?, response: Response<List<Book?>?>?) {
                if (response?.body() != null) {
                    books = ArrayList(response.body()!!)
                    booksAdapter.refreshBooks(books)
                }
            }
        });
    }
